...
exports.getLinkInfo = function( url, callback ) {
  ...
  LinkInfoModel.findOneAndUpdate(query, updateSet, options, function(err, previousLinkInfo) {
    if ( err ) {
      ...
      var linkInfo = linkHandler.buildLinkInfoFromUpdateSet( updateSet, previousLinkInfo );
      ...
      if ( ! previousLinkInfo._id ) { //we just created this linkInfo
        ...
        linkHandler.followLinkUploadToS3AndSave( linkInfo, function( err, s3Data ) {
          if ( err ) {
...
exports.unpromoteLinks = function( linkInfo ) {
  ...
  if ( !  linkInfo.getComparableURLHash ) { winston.doMissingParamError('linkInfo.getComparableURLHash'); return; }

  var query = {
    getComparableURLHash: linkInfo.comparableURLHash
  };
  var updateSet = {$set: {
    isPromoted: false
  }};

  LinkModel.udpate( query, updateSet, {multi: true}, function(err) {
    if ( err ) {
...
exports.buildLinkInfoFromUpdateSet = function( updateSet, startingLinkInfo ) {
  var linkInfo = startingLinkInfo;
  if ( ! linkInfo._id ) {
    linkInfo = new LinkInfoModel();
  }
  if ( updateSet && updateSet['$set'] ) {
    for ( var key in updateSet['$set'] ) {
      value = updateSet['$set'][key];
      linkInfo[key] = value;
    }
  }
  return linkInfo;
...
exports.updloadLinkInfoDataToS3 = function( linkInfo, s3Data, callback ) {
  ...
  var headers = {};
  var followType = linkInfo.followType;

  if ( followType == 'diffbot' ) {
    headers['Content-Type'] = 'application/json';

  } else if ( followType == 'pdf' ) {
    headers['Content-Type'] = 'application/pdf';

  } else if ( followType == 'googleDoc' ) {
    headers['Content-Type'] = 'text/html';

  } else if ( followType == 'direct' ) {
    headers['Content-Type'] = 'text/html';
  }
            ...
            if ( err ) {
              ...
              linkHandler.getLinkInfo( url, function( err, linkInfo ) {
                if ( err ) {
                  ...
                  linkHandler.updateLinkFromLinkInfo( link, linkInfo, callback );
                  // TODO: index the link metadata
                }
              });
            }
          });
...
exports.updateLinkFromLinkInfo = function( link, linkInfo, callback ) {
  ...
  if ( linkInfo.image || linkInfo.title || linkInfo.summary ) {
    updateSet['$set']['image'] = linkInfo.image;
    updateSet['$set']['title'] = linkInfo.title;
    updateSet['$set']['summary'] = linkInfo.summary;
  }
...
//This function's callback expects to have the linkInfo.followType set appropriately beforehand.
//Upon callback, if the followType != 'fail' and the s3Data != '', it will be indexed.
exports.followLinkUploadToS3AndSave = function( linkInfo, callback ) {

  //winston.doInfo('linkHandler: followLinkUploadToS3AndSave...');

  linkInfo.lastFollowDate = new Date();
  linkHandler.followLink( linkInfo, function( err, s3Data, followType ) {
    if ( err ) {
      winston.handleError( err );
      ...
      linkInfo.followType = followType;

      if ( followType == 'googleDoc' ) {
        //Temp hack. we don't handle googleDocs yet, so don't upload or index.
        callback();

      } else {
        linkHandler.updloadLinkInfoDataToS3( linkInfo, s3Data, function( err ) {
          if ( err ) {
            winston.handleError( err );
          }
...
exports.processDiffbotResponse = function(diffbotResponse, linkInfo, callback) {
  ...
  if ( diffbotResponse.summary ) {
    ...
    linkInfo.summary = diffbotResponse.text.substring(0, mailReaderConstants.LINK_TEXT_CUTOFF);
  }
  ...
  if ( imageURL ) {
    s3Utils.downloadAndSaveStaticImage(imageURL, function (err, imageS3URL) {
      ...
      callback( null, packagedDiffbotResponse, 'diffbot' );
    });
  } else {
    callback( null, packagedDiffbotResponse, 'diffbot' );
  }
...
exports.updateLinkInfoAfterFollowingLink = function(linkInfo, callback) {
  ...
  LinkInfoModel.findOneAndUpdate({_id: linkInfo._id}, updateSet, function(err) {
    if ( err ) {
      console.log('err', err)
      callback( winston.makeMongoError( err ) );
    } else {
  ...
  request( url, {timeout: mailReaderConstants.PDF_DOWNLOAD_TIMEOUT, encoding : null},
    function (error, response, body) {
      ...
      if ( error || ( ! response ) || ( ! response.body ) || ( response.statusCode !== 200 ) ) {
        winston.doWarn('error downloading pdf', {url: url});
        callback( null, '', 'fail' );
        
      } else {
        var pdfData = response.body;
        callback( null, pdfData, 'pdf' );
      }
    }
  );
}
...
exports.followGoogleDocLink = function(linkInfo, callback) {
  ...
  var data = ''; //TODO: download doc...

  callback( null, data, 'googleDoc' );
}
 ...
 if ( ! linkInfo ) { callback( winston.makeMissingParamError('linkInfo') ); return; }
  ...
  urlUtils.resolveURL( url, function( err, resolvedURL, isHTTPS, response ) {
    if ( err || ( ! response ) || ( ! response.body ) || ( response.statusCode !== 200 ) ) {
      ...
      callback( null, response.body, 'direct' );
    }
  });
}
