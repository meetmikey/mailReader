...
exports.handleValidURL = function( url, mail, callback ) {
  ...
  linkHandler.lookForDuplicateOnThread( url, mail, function( err, isDuplicateOnThread, needsUpdateToMail, link ) {
    if ( err ) {
      ...
      linkHandler.handleURLOnThread( url, mail, callback );
    }
  });
}
  ...
  linkHandler.updateLinkToMail( link, mail, function(err, havePropertiesChanged, updatedLink) {
    if ( err ) {
      ...
      if ( updatedLink.isFollowed ) {
        indexingHandler.createIndexingJobForDocument( updatedLink, true, false, callback );
...
exports.handleURLOnThread = function( url, mail, callback ) {
...
exports.updateLinkToMail = function( link, mail, callback ) {
  ...
  LinkModel.findOneAndUpdate(filter, updateSet, function(err, updatedLink) {
    if ( err ) {
      ...
      sqsConnect.addMessageToCacheInvalidationQueue (JSON.stringify (invalidateJob), function (err) {
        if (err) {
          callback (err); 
        } else {
