            ...
            linkHandler.followLinkUploadAndSave( linkInfo, userId, function( err, linkFollowData ) {
              if ( err ) {
                ...
                if ( ! linkFollowData ) {
                  ...
                  //Index the linkInfo
                  indexingHandler.indexResource(linkInfo, linkFollowData, linkInfo.comparableURLHash, isLink, function (err) {
                    if ( err ) {
                      winston.handleError( err );
                    }
                  });
                ...
                if ( err ) {
                  ...
                  linkHandler.updateLinkFromLinkInfo( link, linkInfo, function(err) {
                    if ( err ) {
                      ...
                      indexingHandler.indexResourceMetadata(link, mail, parentId, isLink, function (err) {
                        callback();
                      })
                    }
                  });
