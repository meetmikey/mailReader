            ...
            linkHandler.followLinkUploadAndSave( linkInfo, userId, function( err, linkFollowData ) {
              if ( err ) {
                ...
                if ( ! linkFollowData ) {
                  ...
                  indexingHandler.indexResource( linkInfo, linkFollowData, linkInfo.comparableURLHash, isLink );
                }
                ...
                if ( err ) {
                  ...
                  linkHandler.updateLinkFromLinkInfo( link, linkInfo, function(err) {
                    if ( err ) {
                      ...
                      indexingHandler.indexResourceMetadata( link, mail, parentId, isLink );
                      //No callback from indexingHandler.  It's all handled internally there.
                      callback();
                    }
                  });
    ...
    if ( err ) {
      ...
      linkHandler.updateLinkForMail( foundLink, url, mail, function(err) {
        if ( err ) {
          ...
          indexingHandler.updateResourceMetadata( foundLink, mail, foundLink.comparableURLHash, isLink );
          //No callback from indexingHandler.  It's all handled internally there.
          callback( null, true );
        }
      });
...
exports.followLinkUploadAndSave = function( linkInfo, userId, callback ) {
  ...
  //winston.doInfo('linkHandler: followLinkUploadAndSave...');
...
exports.processDiffbotResponse = function(diffbotResponse, linkInfo, callback) {
  ...
  winston.doInfo ('Extracted imageURL', {imageURL : imageURL, rawURL : linkInfo.rawURL});
...
exports.updateLinkInfoAfterFollowingLink = function(linkInfo, callback) {
  ...
  //winston.doInfo('linkHandler: updateLinkInfoAfterFollowingLink...', {linkInfo: linkInfo});
