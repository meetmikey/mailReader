...
exports.unpromoteLinks = function( linkInfo ) {
  ...
  if ( !  linkInfo.getComparableURLHash ) {
    winston.doMissingParamError('linkInfo.getComparableURLHash');
    winston.doInfo('UNPROMOTE MISSING COMPARABLE URL HASH', {linkInfo: linkInfo});
    return;
  }
  ...
  linkHandler.followLink( linkInfo, function( err, s3Data, followType ) {
    if ( err ) {
      ...
      if ( followType == 'googleDoc' ) {
        ...
        var warnData = {followType: followType, url: linkHandler.getRealURL( linkInfo )};
 ...
 if ( ! linkInfo ) { callback( winston.makeMissingParamError('linkInfo') ); return; }
  ...
  urlUtils.resolveURL( url, function( err, resolvedURL, isHTTPS, response ) {
    if ( err || ( ! response ) || ( ! response.body ) || ( response.statusCode !== 200 ) ) {
      ...
      linkInfo.title = linkHandler.extractTitleFromHTML( response.body );
...
exports.extractTitleFromHTML = function( html ) {
  ...
  //I know this is ridiculous but it works, it's safe, and it follows the dom-based output of cheerio...
  if ( $('title')
    && ( $('title').length > 0 )
    && $('title')[0]
    && $('title')[0].children
    && ( $('title')[0].children.length > 0 )
    && $('title')[0].children[0]
    && $('title')[0].children[0].data ) {
