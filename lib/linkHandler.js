    ...
    if ( err ) {
      ...
      LinkInfoModel.findOne({comparableURLHash: comparableURLHash}, function(err, linkInfo) {
        if ( err ) {
          ...
          callback( null, linkInfo ); //let the link extraction process move along... //TODO: is this right? ~Sagar
                ...
                if ( err ) {
                  ...
                  linkHandler.updateLinkFromLinkInfo( link, linkInfo, function(err) {
                    if ( err ) {
                      ...
                      indexingHandler.createIndexingJobForResourceMeta (link, true, function (err) {
                        if (err) { winston.handleError (err); }
                      });
...
exports.followDiffbotLink = function( linkInfo, callback ) {
  ...
  try {
    diffbot.article( diffbotData, function(err, response) {
      ...
      } else if  (linkHandler.isDiffbotResponseSuspicious (response)) {
        winston.doWarn('linkHandler: followDiffbotLink: diffbot suspicious data', 
    ...
    if ( err ) {
      ...
      webUtils.webGet( docMetadataURL, function( err, response ) {
        if ( err ) {
          ...
          winston.doWarn('linkHandler: followGoogleDocLink: no body', warnData);
          callback();

        } else {
          var docMetadata = response.body;
          linkHandler.getGoogleDocImageTitleSummaryAndData( linkInfo, docMetadata, accessToken,
    ...
    if ( ! exportLink ) {
      ...
      webUtils.webGet( exportLink, function(err, response ) {
        if ( err ) {
          ...
          winston.doWarn('linkHandler: getGoogleDocTitleSummaryAndData: no body', {linkInfoId: linkInfo._id});
          callback( null, null, mimeType );
...
exports.followLinkDirectly = function(linkInfo, callback) {
 ...
 if ( ! linkInfo ) { callback( winston.makeMissingParamError('linkInfo') ); return; }
  ...
  webUtils.webGet( url, function( err, response, resolvedURL ) {
    ...
    if ( err ) {
      winston.doWarn( 'linkHandler: followLinkDirectly: error response from webGet', warnData );
      callback();
      
    } else if ( ! response ) {
      winston.doWarn( 'linkHandler: followLinkDirectly: no response from webGet', warnData );
      callback();

    } else if ( ! response.body ) {
      winston.doWarn( 'linkHandler: followLinkDirectly: no response body from webGet', warnData );
      callback();
