  ...
  linkHandler.followLink( linkInfo, userId, function( err, s3Data, mimeType ) {
    if ( err ) {
      ...
      if ( ! s3Data ) {
        ...
        linkHandler.updloadLinkInfoDataToCloud( linkInfo, s3Data, mimeType, function( err ) {
          if ( err ) {
            winston.handleError( err );
          }
    ...
    if ( err ) {
      ...
      if ( ! exportLink ) {
        ...
        request( exportLink, function(err, response, docData ) {
          if ( err ) {
            callback( winston.makeError('export request error', {err: err.toString(), exportLink: exportLink}) );
