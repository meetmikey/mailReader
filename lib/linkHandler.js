    ...
    if ( ( ! matchedURLs ) || ( matchedURLs.length == 0 ) ) {
      ...
      async.waterfall([
        ...
        function(validURLs, waterfallCallback) {
          linkHandler.filterBadLinks(validURLs, parsedMail, waterfallCallback);
        },
        function(validURLs, waterfallCallback) {
...
exports.filterBadLinks = function(urls, parsedMail, callback) {
  
  //TODO: filter out marketing stuff and other bad links...
  var goodURLs = [];
  async.forEach( urls, function(url, forEachCallback) {

    if ( ( ! linkHandler.isFromMarketer(parsedMail) )
      && ( ! linkHandler.containsMarketingText(parsedMail) ) ) {
      goodURLs.push(url);
    }
