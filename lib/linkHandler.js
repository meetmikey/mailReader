...
exports.handleValidURL = function(url, mail, callback) {
  ...
  linkHandler.isPromotable( url, mail, function(err, isPromotable, nonPromotableReason ) {
    if ( err ) {
      ...
      linkHandler.buildAndSaveLink( url, mail, nonPromotableReason, callback);
      ...
      linkHandler.checkAndHandleDuplicateOnThread( url, mail, function( err, isDuplicateOnThread ) {
        if ( err ) {
          ...
          var link = linkHandler.buildLink( url, mail );
          linkHandler.handlePromotableNewLinkOnThread( link, mail.userId, callback );
        }
      });
    }
  });
    ...
    if ( err ) {
      ...
      linkHandler.updateLinkForMail( foundLink, url, mail, function(err) {
        if ( err ) {
          ...
          indexingHandler.createIndexingJobForResourceMeta (foundLink, true, function( err ) {
            if ( err ) {
              callback( err );
...
exports.handlePromotableNewLinkOnThread = function( link, userId, callback ) {
  ...
  link.save( function(err) {
    if ( err ) {
      ...
      linkHandler.getLinkInfo( link.url, userId, function( err, linkInfo ) {
        if ( err ) {
          callback( err );
        ...
        } else if ( ( ! linkInfo.followType ) || ( linkInfo.followType == 'fail' ) ) {
          //This linkInfo followType is failed, so we don't want to index or update.
          callback();
      ...
      LinkInfoModel.findOne({comparableURLHash: comparableURLHash}, function(err, linkInfo) {
        if ( err ) {
          ...
          if ( ! previousLinkInfo._id ) { //we just created this linkInfo
            followLinkUtils.createFollowLinkJob( linkInfo, userId );
          }
...
exports.updateLinkFromLinkInfo = function( link, linkInfo, callback ) {
  ...
  if ( linkInfo.resolvedURL ) {
    link.resolvedURL = linkInfo.resolvedURL;
    updateSet['$set']['resolvedURL'] = linkInfo.resolvedURL;
  }
  if ( linkInfo.followType && linkInfo.followType !== 'fail' ) {
    link.isPromoted = true; //Sagar and Justin decided that we can promote all links when following has succeeded (4/15/2013)
...
exports.buildAndSaveLink = function( url, mail, nonPromotableReason, callback ) {
  ...
  var link = linkHandler.buildLink( url, mail, nonPromotableReason );
  if ( ! link ) {
...
exports.buildLink = function( url, mail, nonPromotableReason ) {
