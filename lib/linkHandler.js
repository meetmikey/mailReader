                ...
                if ( err ) {
                  ...
                  linkHandler.updateLinkFromLinkInfo( link, linkInfo, function(err) {
                    if ( err ) {
                      ...
                      indexingHandler.indexLink( link, mail );
                      //No callback from indexingHandler.  It's all handled internally there.
                      callback();
                    }
                  });
...
exports.isPromotable = function( url, mail, callback ) {
  ...
  } else if ( linkHandler.getContactRatio( contactData ) < mailReaderConstants.CONTACT_RATIO_LINK_PROMOTION_THRESHOLD ) {
...
exports.getContactRatio = function( contactData ) {
  if ( ! contactData ) {
  ...
  if ( denominator == 0 ) {
    winston.doWarn('linkHandler: getContactRatio: zero denominator');
    return 0;
  }
