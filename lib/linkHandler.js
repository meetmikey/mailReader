        ...
        if ( err ) {
          ...
          linkHandler.getLinkInfo(url, function(err, linkInfo) {
            if ( err ) {
              ...
              linkHandler.buildAndSaveLink( url, mail, true, linkInfo, callback );
            }
          });
        }
      });
...
exports.isPromotable = function( url, mail, callback ) {
  ...
  if ( ! mail.sender ) { callback( winston.makeMissingParamError('mail.sender') ); return; }
  if ( ! mail.sender.email ) { callback( winston.makeMissingParamError('mail.sender.email') ); return; }

  if ( mail.senderContactData && ( Object.keys( mail.senderContactData ).length > 0 ) ) {
    contactUtils.getContactData( mail.userId, mail.sender.email, function(err, contactData) {
      if ( err ) {
        ...
        var isPromotable = linkHandler.isPromotableWithData( url, contactData );
...
exports.isPromotableWithData = function( url, contactData ) {
  ...
  if ( ( ! ( contactData.sent > 0 ) )
    ...
    winston.doInfo('linkHandler: isPromotableWithData: url is not promotable because contact sent and corecipient are both zero!', {contactData: contactData});
    return false;
  }

  var urlFilterText = mailReaderConstants.URL_FILTER_TEXT;
  if ( utils.containsSubstringFromArray( url, urlFilterText ) ) {
    winston.doWarn('linkHandler: isPromotable: url is not promotable', {url: url});
    return false;
  }
  return true;
}
