...
exports.handleAttachment = function( parsedMailAttachment, parsedMail, mail, callback ) {
  ...
  if ( ( ! parsedMailAttachment.fileName ) || ( ! parsedMailAttachment.length ) ) {
    ...
    attachmentHandler.lookForDuplicateOnThread (function (err, duplicateAttachment) {
      if (err) {
        callback (err);
          ...
          if (err) {
            ...
            if (err.code == 11000) {
              ...
              AttachmentModel.findOne (filter)
                .exec (function (err, dupeAttachment) {
                  if (err) {
            ...
            indexingHandler.createIndexingJobForResourceMeta( attachment, false, callback );
          }
        });
      }
    });
  ...
  AttachmentInfoModel.findOneAndUpdate(filter, updateSet, options, function(err, previousAttachmentInfo) {
    if ( err ) {
      ...
      AttachmentInfoModel.findOne(filter, function(err, attachmentInfo) {
        ...
        } else if ( previousAttachmentInfo._id ) { 
          //this attachmentInfo was created previously...
    ...
    attachmentHandler.updateAttachmentForMail( duplicateAttachment, mail, parsedMailAttachment,
      function(err, updatedAttachment) {
        if ( err ) {
          ...
          indexingHandler.createIndexingJobForResourceMeta (updatedAttachment, false, callback);
