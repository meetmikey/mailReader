            ...
            async.parallel ([
              ...
              function (asyncCb) {
                ...
                if ( attachmentInfo.isImage 
                  ...
                  imageUtils.pushThumbnailJobToQueue(message.isQuick, thumbnailJob, asyncCb);
                } else {
                  asyncCb();
                }
              }
...
exports.isPromotable = function( parsedMailAttachment ) {
  ...
  } else { //do filename checks...
    var filename = parsedMailAttachment.fileName;
    var filenameLC = filename.toLowerCase();
    if ( utils.endsWith( filenameLC, '.ics' ) ) { //calendar
      return false;

    } else if ( utils.endsWith( filenameLC, '.p7s' ) ) { // weird crypto key
      return false;

    } else if ( utils.endsWith( filenameLC, '.vcf' ) ) { // weird contact card
      return false;

    } else if ( filenameLC === 'winmail.dat' ) { // crappy files
      return false;

    } else if ( filenameLC === 'untitled' ) { // sometimes calendar invites look like this (without an extension)
      return false;

    } else if ( filename === serverCommonConstants.LIKE_EMAIL_ATTACHMENT_FILENAME ) { // our special like email attachments
      return false;

    } else if ( attachmentUtils.isATTFile( filename ) ) { //weird ATT00001..txt files
      return false;
    }
  }
  return true;
