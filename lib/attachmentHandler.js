      ...
      if ( err ) {
        ...
        attachmentHandler.saveAttachment(attachment, function (err) {
          ...
          indexingHandler.indexResourceMetadata (attachment, mail, resourceId, isLink, function (err) {
            console.log ('attachment metadata indexed',  attachment._id)
            callback (err)
          });
        ...
        function(err, updatedAttachment) {
          if ( err ) {
            ...
            indexingHandler.updateResourceMetadata (updatedAttachment, mail, resourceId, isLink, function (err) {
              if (err) { return callback (err); }
              attachmentHandler.updateAttachmentIndexingStatus (updatedAttachment._id)
...
exports.updateAttachmentIndexingStatus = function (attachmentId) {
  AttachmentModel.update ({_id : attachmentId}, {$set : {isIndexed : true}}, function (err, num) {
    if (err) { 
      winston.doError ('could not set indexing state for attachment', {err: err, _id : attachmentId});
    }
    else if (num == 0) {
      winston.doError ('zero records affected when trying to update attachment isIndexed', {_id : attachmentId})
    }
  })
}
