...
exports.uploadAttachment = function(mailAttachment, parsedMail, mailId, userId, callback) {
  ...
  var attachment = new AttachmentModel({
    ...
    , hash: attachmentHandler.getHash( mailAttachment )
  ...
  attachmentHandler.isDuplicate( attachment, function( err, isDuplicate ) {
    if ( err ) {
      ...
      winston.warn('attachmentHandler: uploadAttachment: Duplicate attachment!', {attachment: attachment});
...
exports.getHash = function( mailAttachment ) {
  var shaHash = crypto.createHash('sha256');
  shaHash.update( mailAttachment.content );
  var hash = shaHash.digest('hex');
  if ( ! hash ) {
    winston.warn('attachmentHandler: getHash: blank hash', {mailAttachment: mailAttachment});
  }
