...
exports.buildCheckSaveUploadAttachment = function( parsedMailAttachment, parsedMail, mail, callback ) {
  ...
  if ( ! attachment ) {
    ...
    attachmentHandler.isDuplicate( attachment, function( err, foundDuplicate ) {
      if ( err ) {
        callback(err);
        ...
        // inherit fields
        attachment.attachmentThumbExists = foundDuplicate.attachmentThumbExists;
        attachment.attachmentThumbSkip = foundDuplicate.attachmentThumbSkip;
        attachment.attachmentThumbErr = foundDuplicate.attachmentThumbErr;
              ...
              if (err) {
                ...
                if ( ! attachment.isPromoted ) {
                  ...
                  if (attachment.isImage) {
                    ...
                    var thumbnailJob = {
                      cloudPath : cloudStorageUtils.getAttachmentPath (attachment),
                      isRollover : false,
                  ...
                  //indexingHandler.indexAttachmentAndFile( attachment, parsedMailAttachment, mail );
                  //createIndexingJobForResource = function ( model, isLinkInfo, callback )
                  indexingHandler.createIndexingJobForResource (attachment, false, function (err) {
                    if (err) { winston.handleError (err); }
                  });
      ...
      attachmentHandler.updateAttachmentForMail( foundAttachment, mail, parsedMailAttachment,
        function(err, updatedAttachment) {
          if ( err ) {
            ...
            // TODO: for updates we'll need to know that the original attachment was indexed already for this to go
            // through... so what we should do is...
            indexingHandler.updateAttachment( updatedAttachment, mail );
            //No callback from indexingHandler.  It's all handled internally there.
            callback( null, isDuplicateOnThread );
          }
        }
