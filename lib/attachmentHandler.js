...
exports.handleAttachment = function( parsedMailAttachment, parsedMail, mail, callback ) {
  ...
  winston.info('attachmentHandler: handleAttachment...');
  ...
  attachmentHandler.checkAndHandleDuplicateOnThread( parsedMailAttachment, parsedMail, mail,
    function( err, isDuplicateOnThread ) {
      if ( err ) {
        ...
        winston.info('attachmentHandler: handleAttachment: duplicate attachment was found on thread');
        callback();

      } else { //Normal case: first time we've seen this attachment on this thread for this user.
...
exports.buildCheckSaveUploadAttachment = function( parsedMailAttachment, parsedMail, mail, callback ) {
  ...
  winston.info('attachmentHandler: buildCheckSaveUploadAttachment...');
  ...
  if ( ! attachment ) {
    ...
    attachmentHandler.isDuplicate( attachment, function( err, isDuplicate ) {
      if ( err ) {
        ...
        var winstonWarningData = {attachmentId: attachment._id, hash: attachment.hash, mailId: mail._id, userId: mail.userId, filename: attachment.filename};
        winston.warn('attachmentHandler: buildCheckSaveUploadAttachment: Duplicate attachment!', winstonWarningData);
...
exports.checkAndHandleDuplicateOnThread = function( parsedMailAttachment, parsedMail, mail, callback ) {
  ...
  winston.info('attachmentHandler: checkAndHandleDuplicateOnThread...');
  ...
  AttachmentModel.findOne(duplicateSearchCriteria, function(err, foundAttachment) {
    if ( err ) {
      ...
      var winstonLogMessage = 'attachmentHandler: checkAndHandleDuplicateOnThread: duplicate found, but already from an earlier date';
      winston.info(winstonLogMessage, {foundAttachment: foundAttachment});
      callback(null, true);
    }
  });
}
...
exports.updateAttachmentForMail = function( attachment, mail, parsedMailAttachment, callback ) {
  ...
  winston.info('attachmentHandler: updateAttachmentForMail...');
...
exports.getMailAttachmentHash = function( parsedMailAttachment ) {
  ...
  if ( ! hash ) {
    winston.warn('attachmentHandler: getMailAttachmentHash: blank hash', {parsedMailAttachment: parsedMailAttachment});
  }
