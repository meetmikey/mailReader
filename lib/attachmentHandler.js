...
exports.handleAttachments = function( parsedMail, mail, userId, callback ) {
  if ( parsedMail.attachments && ( parsedMail.attachments.length > 0 ) ) {
    async.forEach( parsedMail.attachments, 
      function(mailAttachment, forEachCallback) {
        attachmentHandler.handleAttachment(mailAttachment, parsedMail, mail, userId, forEachCallback);
      },
      function(err) {
        callback(err);
      }
...
exports.handleAttachment = function(mailAttachment, parsedMail, mail, userId, callback) {
...
/*
  ...
  is there an attachment with this hash and fileSize on this thread?
    yes:
      is our sentDate earlier?
        yes:
...
*/
  ...
  var attachment = attachmentHandler.buildAttachment( mailAttachment, parsedMail, mail );
  ...
  attachmentHandler.isDuplicate( attachment, function( err, isDuplicate ) {
    if ( err ) {
      ...
      winston.warn('attachmentHandler: handleAttachment: Duplicate attachment!', {attachment: attachment});
...
exports.buildAttachment = function( mailAttachment, parsedMail, mail) {
