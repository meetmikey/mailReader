...
exports.tempLog = function( tag, parsedMailAttachment, mail ) {
  var logData = {
      filename: parsedMailAttachment.fileName
    , counter: parsedMailAttachment.counter
    , fileSize: parsedMailAttachment.length
    , checksum: parsedMailAttachment.checksum
    , mailId: mail._id
  };
  winston.doInfo( 'tempLog: ' + tag, logData);
}
...
exports.handleAttachment = function( parsedMailAttachment, parsedMail, mail, callback ) {
  ...
  if ( ( ! parsedMailAttachment.fileName ) || ( ! parsedMailAttachment.length ) ) {
    ...
    attachmentHandler.tempLog( 'handleAttachment: invalid', parsedMailAttachment, mail );
    callback();
    ...
    attachmentHandler.checkAndHandleDuplicateOnThread( parsedMailAttachment, parsedMail, mail,
      function( err, isDuplicateOnThread ) {
        if ( err ) {
          attachmentHandler.tempLog( 'handleAttachment: checkAndHandleDuplicateOnThread: err', parsedMailAttachment, mail );
          callback( err );

        } else if ( isDuplicateOnThread ) {
          attachmentHandler.tempLog( 'handleAttachment: checkAndHandleDuplicateOnThread: isDuplicateOnThread', parsedMailAttachment, mail );
          callback();
...
exports.buildCheckSaveUploadAttachment = function( parsedMailAttachment, parsedMail, mail, callback ) {
  ...
  if ( ! attachment ) {
    attachmentHandler.tempLog( 'buildCheckSaveUploadAttachment: no attachment', parsedMailAttachment, mail );
    ...
    attachmentHandler.isDuplicate( attachment, function( err, isDuplicate ) {
      if ( err ) {
        attachmentHandler.tempLog( 'buildCheckSaveUploadAttachment: isDuplicate err', parsedMailAttachment, mail );
        callback(err);
        ...
        attachmentHandler.saveAttachment(attachment, function (err) {
          if (err) {
            attachmentHandler.tempLog( 'buildCheckSaveUploadAttachment: isDuplicate: saveAttachment: err', parsedMailAttachment, mail );
            callback( err );

          } else if ( ! attachment.isPromoted ) {
            attachmentHandler.tempLog( 'buildCheckSaveUploadAttachment: isDuplicate: saveAttachment: not isPromoted', parsedMailAttachment, mail );
            callback();
            ...
            attachmentHandler.tempLog( 'buildCheckSaveUploadAttachment: isDuplicate: saveAttachment: indexed', parsedMailAttachment, mail );
            callback();
          }
        });

      } else {
        attachmentHandler.tempLog( 'buildCheckSaveUploadAttachment: not isDuplicate: uploadToCloud: CALLED', parsedMailAttachment, mail );

        attachmentHandler.uploadToCloud( attachment, parsedMailAttachment, function(err) {

          attachmentHandler.tempLog( 'buildCheckSaveUploadAttachment: not isDuplicate: uploadToCloud: RETURNED', parsedMailAttachment, mail );

          if ( err ) {
            attachmentHandler.tempLog( 'buildCheckSaveUploadAttachment: not isDuplicate: uploadToCloud: err', parsedMailAttachment, mail );
            callback(err);

          } else {
            attachmentHandler.saveAttachment( attachment, function (err) {
              if (err) {
                attachmentHandler.tempLog( 'buildCheckSaveUploadAttachment: not isDuplicate: uploadToCloud: saveAttachment: err', parsedMailAttachment, mail );
                callback (err);

              } else if ( ! attachment.isPromoted ) {
                attachmentHandler.tempLog( 'buildCheckSaveUploadAttachment: not isDuplicate: uploadToCloud: saveAttachment: not isPromoted', parsedMailAttachment, mail );
                callback();
                ...
                //No callback from indexingHandler.  It's all handled internally there.
                attachmentHandler.tempLog( 'buildCheckSaveUploadAttachment: not isDuplicate: uploadToCloud: saveAttachment: indexed', parsedMailAttachment, mail );
                callback();
              }
            });
          }
