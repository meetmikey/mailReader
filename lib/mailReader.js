...
exports.processMail = function( parsedMail, mailId, userId, callback ) {
  ...
  this.checkAndUpdateMail(parsedMail, mailId, userId, function(err, mail) {
    if ( err ) {
      ...
      winston.warn('mailReader: processMail: no error, but no mail returned frmo checkAndUpdateMail', {mailId: mailId, userId: userId});
      callback();

    } else {
      async.parallel([
        function(parallelCallback) {
          attachmentHandler.handleAttachments(parsedMail, mail, userId, parallelCallback );
        }
        , function(parallelCallback) {
          linkHandler.extractLinks(parsedMail, mail, userId, parallelCallback );
        }], function(err) {
          callback(err);
        }
      );
