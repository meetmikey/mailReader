...
exports.handleMailMessage = function(messageString, callback) {
  ...
  var mailParser = mailReader.getNewMailParser();
...
exports.getNewMailParser = function() {
  var mailParser = new MailParser({
    streamAttachments: mailReaderConstants.STREAM_ATTACHMENTS
  });

  var parsedMailAttachmentsWithBuffers = [];
  ...
  mailParser.on('end', function( parsedMail ) {
    if ( isDone ) {
      ...
      if ( ( ! mailReaderConstants.STREAM_ATTACHMENTS )
        || ( ! parsedMailComplete.attachments )
        || ( ! ( parsedMailComplete.attachments.length > 0 ) )
        || ( parsedMailAttachmentsWithBuffers.length === parsedMailComplete.attachments.length ) ) {

          parsedMailComplete.attachments = parsedMailAttachmentsWithBuffers;
          isDone = true;
          mailReader.runMailProcessor( parsedMailComplete, mailId, userId, callback );
      }
    }
  });
  ...
  mailParser.on('attachment', function( parsedMailAttachment ) {
    if ( isDone ) {
      winston.doWarn('attachment streamed after already done');
      ...
      utils.streamToBuffer( parsedMailAttachment.stream, function(err, attachmentBuffer ) {
        if ( err ) {
          ...
          parsedMailAttachment.content = attachmentBuffer;
          attachmentHandler.validateParsedMailAttachment( parsedMailAttachment, function(err) {
            if ( err ) {
              isDone = true;
              callback( err );

            } else {
              parsedMailAttachmentsWithBuffers.push( parsedMailAttachment );
              if ( parsedMailComplete && ( parsedMailAttachmentsWithBuffers.length == parsedMailComplete.attachments.length ) ) {
...
exports.markMailReaderAsFail = function( mailId, failState, userId, mailReaderError, callback ) {
  mailReader.setMailReaderState( mailId, failState, userId, function(setMailReaderStateErr) {
    ...
    if ( setMailReaderStateErr ) {
      ...
      winston.doInfo('mailReader: handleMailMessage: got error, marked mail softFail', {err: mailReaderError, mailId: mailId});
      ...
      async.parallel([
        ...
        , function(parallelCallback) {
          ...
          if ( err ) {
            ...
            mailUtils.saveMailBody( mail, false, function(err) {
              if ( err ) {
...
exports.checkAndUpdateMail = function( parsedMail, mailId, userId, callback ) {
  ...
  var shardKey = mongoUtils.getShardKeyHash( userId );
  if ( ! shardKey ) { callback( winston.makeMissingParamError('shardKey') ); return; }

  MailModel.findOne({_id: mailId, userId: userId, shardKey: shardKey}, function(err, foundMail) {
    if ( err ) {
