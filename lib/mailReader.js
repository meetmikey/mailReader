...
exports.handleMailMessage = function(messageString, callback) {
  ...
  mailParser.on('end', function( parsedMail ) {
    ...
    mailReader.processMail( parsedMail, mailId, userId, function(err) {
      ...
      if ( err ) { //mail processing failed, so try to mark it 'softFail' as we give up...
        mailReader.markMailReaderAsFailAndCallback (mailId, 'softFail', userId, err, callback);
  ...
  mailReader.downloadMailAndRunParser( mailS3Path, mailParser, inAzure, function (err)  {
    // only callback in error case since mailReader will kick off
    // and handle the dequeue callback
    if (err) {
      mailReader.markMailReaderAsFailAndCallback (mailId, 'hardFail', userId, err, callback);
    }
  });
}

exports.markMailReaderAsFail = function (mailId, failMsg, userId, mailReaderError, callback) {
  ...
  if ( mail.indexState && ( mail.indexState != 'none' ) ) {
    ...
    cloudStorageUtils.putBuffer( buffer, cloudPath, options, useGzip, useAzure, function (err) {
      if (err) {
        ...
        MailModel.update (query, {$set : {bodyInS3 : 'fail'}}, function (err, num) {
          if (err) { 
            winston.doError ('could not update model bodyInS3', err); 
          }
          else if (num == 0) {
        ...
        MailModel.update (query, {$set : {bodyInS3 : 'success'}}, function (err, num) {
          if (err) { 
            winston.doError ('could not update model bodyInS3', err); 
          }
          else if (num == 0) {
