...
exports.handleMailMessage = function(isQuick, messageString, callback) {
  ...
  // preserve whether this came from a fast or slow queue
  message.isQuick = isQuick;

  var mailParser = mailReader.getNewMailParser( message, mailId, userId, callback, function(err) {
...
exports.getNewMailParser = function( message, mailId, userId, mailProcessorCallback, callback ) {
...
exports.runMailProcessor = function( message, parsedMail, mailId, userId, callback ) {
  mailReader.processMail( message, parsedMail, mailId, userId, function(err, mail, mailReaderState) {

    if ( err ) { 
      var errorType = winston.getErrorType (err);
      if ( errorType && errorType === mailReaderConstants.ERROR_TYPE_HARD_FAIL ) {
        mailReader.markMailReaderAsFail(mailId, 'hardFail', userId, err, callback);        
      } else {
        mailReader.markMailReaderAsFail(mailId, 'softFail', userId, err, callback);        
      }
    } else {
      if (mail && mailReaderState !== 'hardFail') {
        //Mail processing succeeded.  Mark the mailReaderState 'done', callback...
        mailReader.setMailReaderState( mailId, 'done', userId, function(setMailReaderStateErr) {
          if ( setMailReaderStateErr ) {
            ...
            mailReader.updateMinMRProcessedDate (userId, mail.gmDate, callback);
          }
        });
      } else {
        callback();
...
exports.validateMessage = function(message, callback) {
  ...
  if ( ! message ) {
    winston.doWarn('mailReader: validateMessage: empty message!');
    callback();
    return false;
...
exports.processMail = function( message, parsedMail, mailId, userId, callback ) {
  ...
  mailReader.checkAndUpdateMail(parsedMail, mailId, userId, function(err, mail, mailReaderState) {
    if ( err ) {
      callback(err);

    } else if ( mailReaderState == 'done' ) {
      //This case indicates that we should not continue processing the mail.
      winston.doWarn('mailReader: processMail: no error, but no mailReaderState already done', {mailId: mailId, userId: userId});
      callback(null, mail, mailReaderState);

    } else if (mailReaderState == 'hardFail' ) {
      winston.doWarn ('mailReader: hardFail, deleting from queue');
      callback(null, mail, mailReaderState);
      ...
      mailUtils.saveMailBody( mail, true, function(err) {
        ...
        async.parallel([
          function(parallelCallback) {
            attachmentHandler.handleAttachments( message, parsedMail, mail, parallelCallback );
          }
          , function(parallelCallback) {
            linkHandler.extractLinks( message, parsedMail, mail, parallelCallback );
        }], function(err) {
          if ( err ) {
            callback(err);
          }
...
//Check for existing mail, callback with no mail if duplicate, otherwise save and callback mail
  ...
  MailModel.findOne({_id: mailId, userId: userId}, function(err, foundMail) {
    if ( err ) {
      ...
      if ( mailReaderState == 'hardFail' ||  mailReaderState == 'done' ) {
        ...
        winston.doWarn('mailReader: checkAndUpdateMail: processing mail with mailReaderState: ' + mailReaderState, {mailId: mailId});
        callback(null, foundMail, mailReaderState);

      } else if ( mailReaderState == 'softFail' && foundMail.tries >= mailReaderConstants.MAX_TRIES_MAILREADER ) {
        var winstonError = winston.makeError('too many retries for mail: setting state to hardFail', {mailId : mailId});
        winston.setErrorType( winstonError, mailReaderConstants.ERROR_TYPE_HARD_FAIL );
        callback( winstonError );

      } else if (mailReader.recipientInBlackList (foundMail.recipients)) {
        var winstonError = winston.makeError('bad recipient: setting state to hardFail', {mailId : mailId});
        winston.setErrorType( winstonError, mailReaderConstants.ERROR_TYPE_HARD_FAIL );
        callback( winstonError );
...
exports.recipientInBlackList = function (recipients) {
  if (!recipients || (recipients && recipients.length == 0)) {
    return false;
  } else {
    // hack: no-reply@coursework breaks mailparser...
    var badRecipients = _.filter (recipients, function (recipient) { 
      return mailReaderConstants.RECIPIENT_BLACKLIST.indexOf (recipient.email) != -1
    });
    return badRecipients.length > 0;
  }
}
...
exports.updateMinMRProcessedDate = function (userId, mailDate, callback) {
  UserModel.update ({_id : userId, minMRProcessedDate : {$gt : mailDate}},
    {$set : {minMRProcessedDate : mailDate}}, 
    function (err) {
      if (err) {
        callback (winston.makeMongoError (err));
      } else {
