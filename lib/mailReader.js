...
exports.runMailProcessor = function( parsedMail, mailId, userId, callback ) {
  mailReader.processMail( parsedMail, mailId, userId, function(err) {
    ...
    if ( err ) { 
      ...
      if (errorType && errorType === 'hardFail') {
  ...
  MailModel.findOne({_id: mailId, userId: userId}, function(err, foundMail) {
    if ( err ) {
      ...
      if ( ( mailReaderState == 'hardFail' ) || ( mailReaderState == 'done' ) ) {
        ...
        callback (winston.makeError ('too many retries for mail: setting state to hardFail', {mailId : mailId, type : 'hardFail'}));

      } else if (mailReader.recipientInBlackList (foundMail.recipients)) {
        callback (winston.makeError ('bad recipient: setting state to hardFail', {mailId : mailId, type : 'hardFail'}));
