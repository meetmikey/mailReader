...
exports.handleMailMessage = function(messageString, callback) {
  ...
  var mailParser = new MailParser({
    streamAttachments: mailReaderConf.STREAM_ATTACHMENTS
  });
  ...
  mailParser.on('end', function( parsedMail ) {
    ...
    if ( mailReaderConf.STREAM_ATTACHMENTS ) {
      if ( parsedMail.attachments && ( parsedMail.attachments.length > 0 )
        && ( parsedMail.attachments.length !== attachmentsWithBuffers.length ) ) {
          var errorData = {parsedMailAttachments: parsedMail.attachments, attachmentsWithBuffers: attachmentsWithBuffers};
          winston.doError( 'streamed attachments not all here yet', errorData );
      } else {
        parsedMail.attachments = attachmentsWithBuffers;
      }
    }
  ...
  mailParser.on('attachment', function( attachment ) {
    utils.streamToBuffer( attachment.stream, function(err, attachmentBuffer ) {
      attachment.content = attachmentBuffer;
      attachmentsWithBuffers.push( attachment );
    });
  });

  mailUtils.downloadMailAndRunParser( mailS3Path, mailParser, inAzure, function (err)  {
