...
exports.indexEmailMetadata = function (attachment, mail, resourceId, callback) {
  ...
  var indexData = indexingHandler.getIndexDataForMail (attachment, mail);
...
exports.updateEmailMetadata = function (oldMailId, attachment, mail, resourceId, callback) {

  winston.info ('old mailId: ' +  String(oldMailId))
  winston.info ('new mailId: ' +  String(mail._id))

  //delete the old version
  indexingHandler.deleteEmailFromIndex (oldMailId)
  indexingHandler.indexEmailMetadata (attachment, mail, resourceId, callback)


}

exports.deleteEmailFromIndex = function (mailId) {
  winston.doInfo ('deleting mail from index', {mailId : String(mailId)})
  elasticSearchClient.deleteDocument ('mail', 'email', mailId)
    .on ('data', function (data) {
      console.log (data)
    })
    .on ('error', function (error){
      winston.doError ('could not delete mail from index', {error : error, mailId : mailId})
    })
    .exec()
...
exports.getIndexDataForMail = function (attachment, mail) {
