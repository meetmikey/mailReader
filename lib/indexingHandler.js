...
exports.indexResource = function (resource, resourceBytes, resourceId, isLink, callback) {
  ...
  var updateCallback = function (err, num) {
    indexingHandler.handleUpdateResponse (err, num);
  }
  ...
  elasticSearchClient.index('mail', 'resource', indexData, options)
    .on('data', function(data) {
      ...
      if (isLink) {
        ...
        AttachmentModel.update ({comparableURLHash : resourceId}, 
          {$set : {indexState : "done"}}, updateCallback);
      }
...
exports.handleUpdateResponse = function (err, num) {
  if (err) { 
    var logData = {comparableURLHash : resourceIdresourceId, err: err, isLink : isLink};
    winston.doError ("Could not update indexState for ", logData);
  }
  else if (num === 0) {
    var logData = {comparableURLHash : resourceId, isLink: isLink};
