  ...
  elasticSearchClient.index('mail', 'resource', indexData, options)
    .on('data', function(data) {
      ...
      if (parsed && parsed.status && parsed.status != 200) {
        if (isLink) {
          indexingHandler.markFailStatusForLink (resourceId, parsed.error);
        }
        else {
        ...
        if (isLink) {
          indexingHandler.markSuccessStatusForLink (resourceId);
        }
        else {
    ...
    .on('error', function (error) {
      ...
      if (isLink) {
        indexingHandler.markFailStatusForLink (resourceId, JSON.stringify(error));
      }
      else {
...
exports.markFailStatusForLink = function (comparableURLHash, error) {
  winston.info ('mark error for linkInfo', {comparableURLHash : comparableURLHash});
...
exports.markSuccessStatusForLink = function (comparableURLHash) {
  winston.info ('mark success for linkInfo', {comparableURLHash : comparableURLHash});
  LinkInfoModel.update ({comparableURLHash : comparableURLHash}, 
    {$set : {indexState : "done"}}, indexingHandler.updateCallback);
}
