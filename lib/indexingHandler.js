...
exports.indexLink = function(link, linkBytes, mail, callback) {
  var resourceId = link.comparableURLHash
  winston.info ('indexing: ' + resourceId)

  /*
  indexingHandler.indexResource (link, linkBytes, resourceId, false, function (err) {
    if (err) { return callback (winston.doError ('Could not index resource', err)); }
...
exports.getIndexDataForResource = function (resource, resourceBytes, isLink) {
  var indexData = {
    'file': resourceBytes.content.toString('base64'),
    'isLink' : isLink,
  }
